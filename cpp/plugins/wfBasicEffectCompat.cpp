...
namespace {
    ...
    public:
        static tjs_error factory(FreeVerbCompat **result, tjs_int count,
            ...
            *result = new FreeVerbCompat();
            if(count > 0)
                (*result)->setmix(
                    VariantToRealDefault(params[0], (*result)->mix_));
            return TJS_S_OK;
        }

        tjs_int64 getinterface() const { return GetInterfaceValue(); }
        tjs_real geteffectMix() const { return mix_; }
        void seteffectMix(tjs_real value) {
            mix_ = std::clamp(value, 0.0, 1.0);
        }
        ...
        static tjs_error setEffectMixCallback(tTJSVariant *result,
                                              tjs_int count,
                                              tTJSVariant **params,
                                              iTJSDispatch2 *objthis) {
            auto *self = ncbInstanceAdaptor<FreeVerbCompat>::GetNativeInstance(
                objthis, true);
            if(!self)
                return TJS_E_FAIL;
            if(count < 1)
    ...
    public:
        ...
        tjs_int getbandCount() const {
            return static_cast<tjs_int>(bands_.size());
        }
        ...
        static tjs_error setCallback(tTJSVariant *result, tjs_int count,
                                     ...
                                     iTJSDispatch2 *objthis) {
            auto *self =
                ncbInstanceAdaptor<GraphicEqualizerCompat>::GetNativeInstance(
                    objthis, true);
            if(!self)
                return TJS_E_FAIL;
            if(count < 2)
        ...
        static tjs_error setParamsCallback(tTJSVariant *, tjs_int count,
            ...
            if(count >= 1)
                self->Resize(
                    VariantToIntDefault(params[0], self->getbandCount()));
    ...
    public:
        static tjs_error factory(Derived **result, tjs_int count,
            ...
            *result = new Derived();
            if(count >= 1)
                (*result)->setdelay(
                    VariantToRealDefault(params[0], (*result)->delay_));
            if(count >= 2)
                (*result)->setmaxDelay(
        ...
        void setdelay(tjs_real value) {
            delay_ = std::max<tjs_real>(value, 0.0);
        }
        ...
        static tjs_error setDelayCallback(tTJSVariant *result, tjs_int count,
            ...
            if(count >= 2)
                self->setmaxDelay(
                    VariantToRealDefault(params[1], self->maxDelay_));
            if(result)
        ...
        static tjs_error setParamsCallback(tTJSVariant *, tjs_int count,
            ...
            if(count >= 3)
                self->setfeedback(
                    VariantToRealDefault(params[2], self->feedback_));
            if(count >= 4)
    ...
    public:
        static tjs_error factory(GainLimit **result, tjs_int count,
            ...
            *result = new GainLimit();
            if(count >= 1)
                (*result)->setgain(
                    VariantToRealDefault(params[0], (*result)->gain_));
            if(count >= 2)
                (*result)->setlimit(
                    VariantToRealDefault(params[1], (*result)->limit_));
            return TJS_S_OK;
        }
        ...
        static tjs_error setGainCallback(tTJSVariant *result, tjs_int count,
                                         ...
                                         iTJSDispatch2 *objthis) {
            auto *self =
                ncbInstanceAdaptor<GainLimit>::GetNativeInstance(objthis, true);
            if(!self)
                return TJS_E_FAIL;
            if(count < 1)
                return TJS_E_BADPARAMCOUNT;
        ...
        static tjs_error setParamsCallback(tTJSVariant *, tjs_int count,
            ...
            if(count >= 4)
                self->setthreshold(
                    VariantToRealDefault(params[3], self->threshold_));
            if(count >= 5)
