...
struct layerExAreaAverage {
    static tjs_error TJS_INTF_METHOD stretchCopyAA(tTJSVariant *result,
        ...
        dPitch = (tjs_int)val;
        objthis->PropGet(0, TJS_W("mainImageBufferForWrite"), NULL, &val,
                         objthis);
        ...
        if(dLeft + dWidth > dImageWidth) {
            ...
            sWidth =
                (tjs_int)((tjs_real)sWidth * ((tjs_real)dw / (tjs_real)dWidth));
            dWidth = dw;
        }
        if(sLeft + sWidth > sImageWidth) {
            tjs_int sw = sImageWidth - sLeft;
            dWidth =
                (tjs_int)((tjs_real)dWidth * ((tjs_real)sw / (tjs_real)sWidth));
            sWidth = sw;
        }
        if(dTop + dHeight > dImageHeight) {
            tjs_int dh = dImageHeight - dTop;
            sHeight = (tjs_int)((tjs_real)sHeight *
                                ((tjs_real)dh / (tjs_real)dHeight));
            dHeight = dh;
        }
        if(sTop + sHeight > sImageHeight) {
            tjs_int sh = sImageHeight - sTop;
            dHeight = (tjs_int)((tjs_real)dHeight *
                                ((tjs_real)sh / (tjs_real)sHeight));
            sHeight = sh;
        }
        ...
        for(tjs_int y = 0; y < dHeight; y++) {
            tjs_uint32 *outpixel =
                (tjs_uint32 *)(dBuffer + (y + dTop) * dPitch) + dLeft;
            for(tjs_int x = 0; x < dWidth; x++) {
                ...
                for(tjs_int ay = sy; ay < ey; ay++) {
                    tjs_uint32 *inpixel =
                        (tjs_uint32 *)(sBuffer + ay * sPitch) + sx;
                ...
                *outpixel = ((((tjs_int)a) & 0xFF) << 24) |
                    ((((tjs_int)r) & 0xFF) << 16) |
                    ((((tjs_int)g) & 0xFF) << 8) | (((tjs_int)b) & 0xFF);
                outpixel++;
            }
        }
