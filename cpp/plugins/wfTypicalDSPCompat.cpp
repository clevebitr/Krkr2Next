...
namespace {
    ...
    public:
        ...
        static tjs_error setParamsCallback(tTJSVariant *result, tjs_int count,
            ...
            auto *self = ncbInstanceAdaptor<WaveDSPFilter>::GetNativeInstance(
                objthis, true);
            if(!self)
                return TJS_E_FAIL;
    ...
    private:
        void ApplyParams(tjs_int count, tTJSVariant **params) {
            ...
            if(count > numericBase + 1)
                frequency_ =
                    VariantToRealDefault(params[numericBase + 1], frequency_);
            ...
            if(count > numericBase + 4)
                ripple_ =
                    VariantToRealDefault(params[numericBase + 4], ripple_);
            if(count > numericBase + 5)
                bandwidth_ =
                    VariantToRealDefault(params[numericBase + 5], bandwidth_);
            if(count > numericBase + 6)
                sampleRate_ =
                    VariantToIntDefault(params[numericBase + 6], sampleRate_);
            if(count > numericBase + 7)
                channels_ =
                    VariantToIntDefault(params[numericBase + 7], channels_);
        }
...
NCB_REGISTER_CLASS(WaveDSPFilter) {
    ...
    RawCallback(TJS_W("type"), &Class::getTypeProperty, &Class::setTypeProperty,
                0);
