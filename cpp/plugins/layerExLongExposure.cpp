...
struct LongExposure {
    ~LongExposure() { term(); }
    LongExposure(iTJSDispatch2 *obj) :
        self(obj), buffer(0), width(0), height(0) {}

    void init() {
        term();
        if(!GetLayerSize(self, width, height))
            TVPThrowExceptionMessage(
                TJS_W("LongExposure.init: invalid layer image"));
    ...
    void snap() {
        if(!buffer)
            TVPThrowExceptionMessage(
                TJS_W("LongExposure.snap: not initialized"));
        size_t curw = 0, curh = 0;
        if(!GetLayerSize(self, curw, curh))
            TVPThrowExceptionMessage(
                TJS_W("LongExposure.snap: invalid layer image"));
        if(curw != width || curh != height)
            TVPThrowExceptionMessage(
                TJS_W("LongExposure.snap: invalid layer size"));
    ...
    struct MinMaxRGBA {
        ...
        inline BYTE getNormalizeR(DWORD n) {
            return getNormalize(n, _min.r, _max.r);
        }
        inline BYTE getNormalizeG(DWORD n) {
            return getNormalize(n, _min.g, _max.g);
        }
        inline BYTE getNormalizeB(DWORD n) {
            return getNormalize(n, _min.b, _max.b);
        }
        inline BYTE getNormalizeA(DWORD n) {
            return getNormalize(n, _min.a, _max.a);
        }
    ...
    tTJSVariant stat() {
        if(!buffer)
            TVPThrowExceptionMessage(
                TJS_W("LongExposure.stat: not initialized"));

        MinMaxRGBA m;
        if(!_stat(m))
            TVPThrowExceptionMessage(
                TJS_W("LongExposure.stat: invalid layer image"));
    ...
    void copy(tTJSVariant v) {
        if(!buffer)
            TVPThrowExceptionMessage(
                TJS_W("LongExposure.copy: not initialized"));
        size_t curw = 0, curh = 0;
        if(!GetLayerSize(self, curw, curh))
            TVPThrowExceptionMessage(
                TJS_W("LongExposure.copy: invalid layer image"));
        if(curw != width || curh != height)
            TVPThrowExceptionMessage(
                TJS_W("LongExposure.copy: invalid layer size"));
        ...
        switch(v.Type()) {
            case tvtVoid:
                if(!_stat(m))
                    TVPThrowExceptionMessage(
                        TJS_W("LongExposure.copy: stat failed"));
                break;
            case tvtObject: {
...
private:
    ...
    static bool GetLayerSize(iTJSDispatch2 *lay, size_t &w, size_t &h) {
        ...
        if(!LayerPropGet(lay, hasImage) ||
           ...
           (lh = LayerPropGet(lay, imageHeight)) <= 0)
            return false;
        w = (size_t)lw;
        h = (size_t)lh;
        return true;
    ...
    static bool GetLayerImage(iTJSDispatch2 *lay, const BYTE *&ptr,
                              long &pitch) {
        ...
        if((lpitch = LayerPropGet(lay, mainImageBufferPitch)) == 0 ||
           (lptr = LayerPropGet(lay, mainImageBuffer)) == 0)
            return false;
        pitch = (long)lpitch;
    ...
    static bool GetLayerImageForWrite(iTJSDispatch2 *lay, BYTE *&ptr,
                                      long &pitch) {
        static ttstr mainImageBufferPitch(TJS_W("mainImageBufferPitch"));
        ...
        tTVInteger lpitch, lptr;
        if((lpitch = LayerPropGet(lay, mainImageBufferPitch)) == 0 ||
           (lptr = LayerPropGet(lay, mainImageBufferForWrite)) == 0)
            return false;
        pitch = (long)lpitch;
    ...
    static tTVInteger LayerPropGet(iTJSDispatch2 *lay, ttstr &prop,
                                   tTVInteger defval = 0) {
        if(!LayerClass) {
            tTJSVariant var;
        ...
        tTJSVariant val;
        return (TJS_FAILED(LayerClass->PropGet(0, prop.c_str(), prop.GetHint(),
                                               &val, lay)))
            ? defval
            : val.AsInteger();
    }
...
NCB_GET_INSTANCE_HOOK(LongExposure){
    NCB_GET_INSTANCE_HOOK_CLASS(){} ~NCB_GET_INSTANCE_HOOK_CLASS(){} NCB_INSTANCE_GETTER(
        objthis){ ClassT *obj = GetNativeInstance(objthis);
if(!obj)
