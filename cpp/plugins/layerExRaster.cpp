...
struct layerExRaster : public layerExBase {
    ...
    void copyRaster(tTJSVariant layer, int maxh, int lines, int cycle,
                    tjs_int64 time) {
        ...
        unsigned char *buffer;
        {
            ...
            height = (tjs_int)var;
            layerobj->PropGet(0, TJS_W("mainImageBuffer"), NULL, &var,
                              layerobj);
            buffer = (unsigned char *)(tjs_intptr_t)(tTVInteger)var;
            layerobj->PropGet(0, TJS_W("mainImageBufferPitch"), NULL, &var,
                              layerobj);
            pitch = (tjs_int)var;
        }
        ...
        for(tjs_int n = 0; n < _clipHeight; n++, rad += omega) {
            ...
            if(d >= 0) {
                ...
                const tjs_uint32 *src =
                    (const tjs_uint32 *)(buffer + n * pitch);
                ...
                int w = _clipWidth + d;
                const tjs_uint32 *src =
                    (const tjs_uint32 *)(buffer + n * pitch) - d;
...
NCB_ATTACH_CLASS_WITH_HOOK(layerExRaster, Layer) { NCB_METHOD(copyRaster); }
