//---------------------------------------------------------------------------
// json.dll —— 给内建 Scripts 类加 JSON 编解码
//---------------------------------------------------------------------------
// 移植自 AetherKiri cpp/plugins/json/jsonPlugin.cpp（同源实现，逐行对照）。
//
// 为什么要它：三个验收游戏里有两个会 Plugins.link("json.dll")，而本仓库原先把
// json/ 子目录注释在构建之外（krkr2 / PocketKrKr / KrKr2-Next / AetherKiri 全都
// 是注释状态；AetherKiri 的可用做法是把源文件**直接**加进父目标，本仓库同样处理）。
// 本仓库旧版 jsonPlugin.cpp 依赖 TVPLoadText/TVPSaveText，而核心并没有这两个函数
// （CI 实测：use of undeclared identifier）—— 这一版改用
// TVPCreateTextStreamForRead/Write，依赖全部在核心中已有。
//---------------------------------------------------------------------------
#include "ncbind.hpp"
#include "tp_stub.h"
