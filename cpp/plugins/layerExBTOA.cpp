...
static tjs_uint32 mainImageBufferHint, mainImageBufferPitchHint,
    mainImageBufferForWriteHint;
static tjs_uint32 provinceImageBufferHint, provinceImageBufferPitchHint,
    provinceImageBufferForWriteHint;
...
static bool GetLayerSize(iTJSDispatch2 *lay, tjs_int32 &w, tjs_int32 &h,
                         tjs_int32 &pitch) {
    ...
    tTJSVariant val;
    if(TJS_FAILED(layerClass->PropGet(0, TJS_W("hasImage"), &hasImageHint, &val,
                                      lay)) ||
       (val.AsInteger() == 0))
        return false;

    if(TJS_FAILED(layerClass->PropGet(0, TJS_W("imageWidth"), &imageWidthHint,
                                      &val, lay)))
        return false;
    w = (tjs_int32)val.AsInteger();

    if(TJS_FAILED(layerClass->PropGet(0, TJS_W("imageHeight"), &imageHeightHint,
                                      &val, lay)))
        return false;
    h = (tjs_int32)val.AsInteger();

    if(TJS_FAILED(layerClass->PropGet(0, TJS_W("mainImageBufferPitch"),
                                      &mainImageBufferPitchHint, &val, lay)))
        return false;
...
static bool GetLayerBufferAndSize(iTJSDispatch2 *lay, tjs_int32 &w,
                                  tjs_int32 &h, WrtRefT &ptr,
                                  tjs_int32 &pitch) {
    iTJSDispatch2 *layerClass = getLayerClass();
    if(!lay || TJS_FAILED(lay->IsInstanceOf(0, 0, 0, TJS_W("Layer"), lay)))
        return false;
    ...
    if(TJS_FAILED(layerClass->PropGet(0, TJS_W("mainImageBufferForWrite"),
                                      &mainImageBufferForWriteHint, &val, lay)))
        return false;
...
static bool GetClipSize(iTJSDispatch2 *lay, tjs_int32 &l, tjs_int32 &t,
                        tjs_int32 &w, tjs_int32 &h, tjs_int32 &pitch) {
    iTJSDispatch2 *layerClass = getLayerClass();
    if(!lay || TJS_FAILED(lay->IsInstanceOf(0, 0, 0, TJS_W("Layer"), lay)))
        return false;
    ...
    if(TJS_FAILED(
           layerClass->PropGet(0, TJS_W("clipLeft"), &clipLeftHint, &val, lay)))
        return false;
    l = (tjs_int32)val.AsInteger();
    if(TJS_FAILED(
           layerClass->PropGet(0, TJS_W("clipTop"), &clipTopHint, &val, lay)))
        return false;
...
static bool GetClipBufferAndSize(iTJSDispatch2 *lay, tjs_int32 &l, tjs_int32 &t,
                                 tjs_int32 &w, tjs_int32 &h, WrtRefT &ptr,
                                 tjs_int32 &pitch) {
    iTJSDispatch2 *layerClass = getLayerClass();
    if(!lay || TJS_FAILED(lay->IsInstanceOf(0, 0, 0, TJS_W("Layer"), lay)))
        return false;
...
static tjs_error TJS_INTF_METHOD copyRightBlueToLeftAlpha(tTJSVariant *result,
                                                          tjs_int numparams,
                                                          tTJSVariant **param,
                                                          iTJSDispatch2 *lay) {
    WrtRefT dbuf = 0;
...
static tjs_error TJS_INTF_METHOD copyBottomBlueToTopAlpha(tTJSVariant *result,
                                                          tjs_int numparams,
                                                          tTJSVariant **param,
                                                          iTJSDispatch2 *lay) {
    WrtRefT dbuf = 0;
...
static tjs_error TJS_INTF_METHOD fillAlpha(tTJSVariant *result,
                                           tjs_int numparams,
                                           tTJSVariant **param,
                                           iTJSDispatch2 *lay) {
    WrtRefT dbuf = 0;
...
static tjs_error TJS_INTF_METHOD copyAlphaToProvince(tTJSVariant *result,
                                                     tjs_int numparams,
                                                     tTJSVariant **param,
                                                     iTJSDispatch2 *lay) {
    iTJSDispatch2 *layerClass = getLayerClass();
    ...
    ReadRefT sbuf = 0;
    WrtRefT dbuf = 0;
    tjs_int32 l, t, w, h, spitch, dpitch, threshold = -1, matched = 1,
                                          otherwise = 0;
    ...
    if(TJS_FAILED(layerClass->PropGet(0, TJS_W("provinceImageBufferForWrite"),
                                      &provinceImageBufferForWriteHint, &val,
                                      lay)) ||
    ...
    if(TJS_FAILED(layerClass->PropGet(0, TJS_W("provinceImageBufferPitch"),
                                      &provinceImageBufferPitchHint, &val,
                                      lay)) ||
...
static inline bool CalcClipArea(tjs_int32 &dx, tjs_int32 &dy,
                                const tjs_int32 diw, const tjs_int32 dih,
                                tjs_int32 &sx, tjs_int32 &sy,
                                const tjs_int32 siw, const tjs_int32 sih,
...
static inline iTJSDispatch2 *
GetSrcDstLayerInfo(iTJSDispatch2 *src, tjs_int32 &siw, tjs_int32 &sih,
                   tjs_int32 &spitch, ReadRefT &sbuf, iTJSDispatch2 *dst,
                   tjs_int32 &dl, tjs_int32 &dt, tjs_int32 &diw, tjs_int32 &dih,
                   tjs_int32 &dpitch, WrtRefT &dbuf) {
    iTJSDispatch2 *layerClass = getLayerClass();
    tTJSVariant val;
    ...
    if(TJS_FAILED(layerClass->PropGet(0, TJS_W("mainImageBuffer"),
                                      &mainImageBufferHint, &val, src)))
        sbuf = 0;
    else
        sbuf = reinterpret_cast<ReadRefT>(val.AsInteger());

    if(TJS_FAILED(layerClass->PropGet(0, TJS_W("mainImageBufferForWrite"),
                                      &mainImageBufferForWriteHint, &val, dst)))
        dbuf = 0;
    else
    ...
    if(TJS_FAILED(layerClass->PropGet(0, TJS_W("mainImageBufferPitch"),
                                      &mainImageBufferPitchHint, &val, dst)))
...
static tjs_error TJS_INTF_METHOD clipAlphaRect(tTJSVariant *result,
                                               tjs_int numparams,
                                               tTJSVariant **param,
                                               iTJSDispatch2 *dst) {
    ...
    iTJSDispatch2 *layerClass = GetSrcDstLayerInfo(
        src, siw, sih, spitch, sbuf, dst, dl, dt, diw, dih, dpitch, dbuf);
    {
        tTJSVariant val;
        if(TJS_FAILED(
               layerClass->PropGet(0, TJS_W("type"), &typeHint, &val, dst)))
            return TJS_E_FAIL;
    ...
    if(CalcClipArea(dx, dy, diw, dih, sx, sy, siw, sih, w, h))
        goto none;

    {
        ...
        if(!addalpha) {
            if(clr) {
                for(y = 0; y < dy; y++)
                    for((x = 0, p = dbuf + y * dpitch + 3); x < diw;
                        x++, p += 4)
                        *p = clrval;
                for(y = dy + h; y < dih; y++)
                    for((x = 0, p = dbuf + y * dpitch + 3); x < diw;
            ...
            for(y = 0; y < h; y++) {
                if(clr)
                    for((x = 0, p = dbuf + (y + dy) * dpitch + 3); x < dx;
                        x++, p += 4)
                        *p = clrval;
            ...
            if(clr && clrval < 255) {
                if(!clrval) {
                    for(y = 0; y < dy; y++)
                        for((x = 0, pp = (PixelT *)(dbuf + y * dpitch));
                            x < diw; x++)
                            *pp++ = 0;
                    for(y = dy + h; y < dih; y++)
                        for((x = 0, pp = (PixelT *)(dbuf + y * dpitch));
                            ...
                            *pp++ = 0;
                    for(y = 0; y < h; y++) {
                        for((x = 0, pp = (PixelT *)(dbuf + (y + dy) * dpitch));
                            x < dx; x++)
                            *pp++ = 0;
                        pp = (PixelT *)(dbuf + (y + dy) * dpitch + (dx * 4));
                        q = sbuf + (y + sy) * spitch + 3 + (sx * 4);
                        for(x = 0; x < w; x++, q += 4) {
                            *pp = ClipAddAlpha(*pp, (PixelT)*q);
                            pp++;
                        }
                        for(x = dx + w; x < diw; x++)
                            *pp++ = 0;
    ...
    if(clr) {
        layObj.FuncCall(0, TJS_W("update"), &updateHint, NULL, dl, dt, diw,
                        dih);
    } else {
        layObj.FuncCall(0, TJS_W("update"), &updateHint, NULL, dl + dx, dt + dy,
                        w, h);
    }
    return TJS_S_OK;
none:
...
static tjs_error TJS_INTF_METHOD overwrapRect(tTJSVariant *result,
                                              tjs_int numparams,
                                              tTJSVariant **param,
                                              iTJSDispatch2 *dst) {
    ncbPropAccessor layObj(dst);
    ...
    if(numparams >= 8 && param[7]->Type() != tvtVoid) {
        tjs_int32 n = (tjs_int32)param[7]->AsInteger();
        if(n >= 0 && n < 256)
            threshold = (unsigned char)(n);
    }
    if(w <= 0 || h <= 0)
        return TJS_E_INVALIDPARAM;

    GetSrcDstLayerInfo(src, siw, sih, spitch, sbuf, dst, dl, dt, diw, dih,
                       dpitch, dbuf);
    ...
    for(tjs_int32 y = 0; y < h; y++) {
        ...
        for(tjs_int32 x = 0; x < w; x++, p += 4, q += 4) {
            if(q[3] >= threshold)
                *(reinterpret_cast<PixelT *>(p)) =
                    *(reinterpret_cast<const PixelT *>(q));
        }
    }
    return TJS_S_OK;
}
...
static tjs_error TJS_INTF_METHOD fillByProvince(tTJSVariant *result,
                                                tjs_int numparams,
                                                tTJSVariant **param,
                                                iTJSDispatch2 *lay) {
    iTJSDispatch2 *layerClass = getLayerClass();
    ...
    {
        tTJSVariant val;
        if(TJS_FAILED(layerClass->PropGet(0, TJS_W("provinceImageBuffer"),
                                          &provinceImageBufferHint, &val,
                                          lay)) ||
           (sbuf = reinterpret_cast<ReadRefT>(val.AsInteger())) == NULL)
...
static tjs_error TJS_INTF_METHOD fillToProvince(tTJSVariant *result,
                                                tjs_int numparams,
                                                tTJSVariant **param,
                                                iTJSDispatch2 *lay) {
    iTJSDispatch2 *layerClass = getLayerClass();
    ...
    for(int y = 0; y < dh; y++) {
        ...
        WrtRefT p = dbuf;
        for(int x = 0; x < dw; x++) {
            if((allmatch || ((*q & 0xFFFFFF) == color)) &&
               ((tjs_int32)(*q >> 24)) >= threshold_val) {
                *p = index;
                if(minx < 0 || minx > x)
                    minx = x;
                if(miny < 0 || miny > y)
    ...
    if(result) {
        ...
        if(minx >= 0) {
            ...
            if(arr) {
                tTJSVariant num;
                num = (minx + l);
                arr->PropSetByNum(TJS_MEMBERENSURE, 0, &num, arr);
                num = (miny + t);
                arr->PropSetByNum(TJS_MEMBERENSURE, 1, &num, arr);
                num = (maxx + l - minx + 1);
                arr->PropSetByNum(TJS_MEMBERENSURE, 2, &num, arr);
                num = (maxy + t - miny + 1);
                arr->PropSetByNum(TJS_MEMBERENSURE, 3, &num, arr);
