...
namespace krkr2 {
    namespace HWY_NAMESPACE {
        ...
        /// Broadcast alpha channel from BGRA pixels to all channels of each
        /// pixel. Input:  {B0,G0,R0,A0, B1,G1,R1,A1, B2,G2,R2,A2, B3,G3,R3,A3}
        ...
        HWY_INLINE hn::Vec<D8> BroadcastAlpha(D8 d8, hn::Vec<D8> pixels) {
            ...
            const auto indices =
                hn::Dup128VecFromValues(d8, 3, 3, 3, 3, // pixel 0: replicate A0
        ...
        HWY_INLINE hn::Vec<D16> BlendChannelU16(D16 d16, hn::Vec<D16> vd,
                                                hn::Vec<D16> vs,
                                                hn::Vec<D16> va) {
            // diff = s - d  (may be negative, but u16 wrapping is fine with the
            // mask)
        ...
        /// alpha_vec must already be broadcast (all 4 channels of each pixel =
        /// alpha).
        HWY_INLINE hn::Vec<D8> BlendPixels(D8 d8, hn::Vec<D8> dest,
                                           hn::Vec<D8> src, hn::Vec<D8> alpha) {
        ...
        HWY_INLINE hn::Vec<D8> ApplyHDA(D8 d8, hn::Vec<D8> blended,
                                        hn::Vec<D8> dest) {
        ...
        HWY_INLINE hn::Vec<D8> ScaleAlphaByOpacity(D8 d8,
                                                   hn::Vec<D8> alpha_broadcast,
                                                   hn::Vec<D8> opa_vec) {
            ...
            auto a_lo =
                hn::PromoteTo(d16, hn::LowerHalf(half, alpha_broadcast));
