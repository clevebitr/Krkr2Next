...
namespace krkr2 {
    namespace HWY_NAMESPACE {
         ...
         *
         * IMPORTANT: We must >>8 each product BEFORE adding, to avoid u16
         * overflow (s*a + d*inv_a can reach 130050 > 65535).
         */
        static HWY_INLINE auto
        BlendChannel(hn::ScalableTag<uint16_t> d16,
        ...
        void AlphaBlend_HWY(tjs_uint32 *dest, const tjs_uint32 *src,
                            tjs_int len) {
        ...
        void AlphaBlend_HDA_HWY(tjs_uint32 *dest, const tjs_uint32 *src,
                                tjs_int len) {
            const hn::ScalableTag<uint8_t> d8;
            const hn::Repartition<uint16_t, decltype(d8)> d16;
            const size_t N_PIXELS = hn::Lanes(d8) / 4;
            ...
            for(; i + static_cast<tjs_int>(N_PIXELS) <= len; i += N_PIXELS) {
                ...
                const auto half = hn::Half<decltype(d8)>();
                auto r_lo = BlendChannel(
                    d16, hn::PromoteTo(d16, hn::LowerHalf(half, vs)),
        ...
        void AlphaBlend_o_HWY(tjs_uint32 *dest, const tjs_uint32 *src,
            ...
            for(; i + static_cast<tjs_int>(N_PIXELS) <= len; i += N_PIXELS) {
                ...
                auto a_lo = hn::ShiftRight<8>(hn::Mul(
                    hn::PromoteTo(d16, hn::LowerHalf(half, va_raw)), vopa16));
                auto a_hi = hn::ShiftRight<8>(
                    hn::Mul(hn::PromoteUpperTo(d16, va_raw), vopa16));
