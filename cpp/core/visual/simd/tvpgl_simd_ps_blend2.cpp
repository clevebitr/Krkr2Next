...
namespace krkr2 {
    namespace HWY_NAMESPACE {
        ...
        static HWY_INLINE tjs_uint32 PsAlphaBlendScalar(tjs_uint32 d,
                                                        tjs_uint32 s,
                                                        tjs_uint32 a) {
        ...
        ApplyHDA(hn::ScalableTag<uint8_t> d8,
                 ...
                 hn::Vec<hn::ScalableTag<uint8_t>> vd) {
            auto mask = hn::Dup128VecFromValues(
                d8, 0, 0, 0, 0xFF, 0, 0, 0, 0xFF, 0, 0, 0, 0xFF, 0, 0, 0, 0xFF);
            return hn::Or(hn::AndNot(mask, result), hn::And(mask, vd));
        }

        // =========================================================================
        // Overlay core: per channel, if d < 128: (2*d*s)/255, else: 2*(d+s) -
        // (2*d*s)/255 - 255 (exact integer /255, matching TVPPsTableOverlay
        // built in TVPPsMakeTable) SIMD: compute m = floor(2*s*d/255) exactly
        // via
        //   p = s*d ; k = p>>7 ; t = k + 2*(p & 127) ; m = k + (t>=255) +
        //   (t>=510)
        // =========================================================================
        static HWY_INLINE hn::Vec<hn::ScalableTag<uint8_t>>
        OverlayCore(hn::ScalableTag<uint8_t> d8,
                    hn::Vec<hn::ScalableTag<uint8_t>> vd,
                    hn::Vec<hn::ScalableTag<uint8_t>> vs) {
            ...
            auto m_lo =
                hn::Add(k_lo,
                        hn::Add(hn::IfThenElse(hn::Not(hn::Lt(t_lo, v255)), v1,
                                               hn::Zero(d16)),
                                hn::IfThenElse(hn::Not(hn::Lt(t_lo, v510)), v1,
                                               hn::Zero(d16))));
            auto m_hi =
                hn::Add(k_hi,
                        hn::Add(hn::IfThenElse(hn::Not(hn::Lt(t_hi, v255)), v1,
                                               hn::Zero(d16)),
                                hn::IfThenElse(hn::Not(hn::Lt(t_hi, v510)), v1,
                                               hn::Zero(d16))));

            // Path 1 (d < 128): m  (& 0xFF so demote truncates like the uchar
            // table)
            auto path1 = hn::OrderedDemote2To(d8, hn::And(m_lo, v255),
                                              hn::And(m_hi, v255));

            // Path 2 (d >= 128): 2*(d+s) - m - 255  (& 0xFF reproduces the
            // unsigned-char scalar table truncation when the value exceeds 255)
            ...
            auto path2 = hn::OrderedDemote2To(d8, hn::And(p2_lo, v255),
                                              hn::And(p2_hi, v255));
        ...
        static HWY_INLINE hn::Vec<hn::ScalableTag<uint8_t>>
        HardLightCore(hn::ScalableTag<uint8_t> d8,
                      hn::Vec<hn::ScalableTag<uint8_t>> vd,
                      hn::Vec<hn::ScalableTag<uint8_t>> vs) {
            ...
            // HardLight: condition on s (not d); value formula identical
            // (product symmetric)
        ...
        static HWY_INLINE hn::Vec<hn::ScalableTag<uint8_t>>
        ExclusionCore(hn::ScalableTag<uint8_t> d8,
                      hn::Vec<hn::ScalableTag<uint8_t>> vd,
                      hn::Vec<hn::ScalableTag<uint8_t>> vs) {
...
#define MAKE_PS_4V(Name, simd_blend_expr)                                      \
    void Ps##Name##Blend_HWY(tjs_uint32 *dest, const tjs_uint32 *src,          \
        ...
        /* scalar NORM writes the alpha byte as-is (partial ps_alpha_blend     \
           only touches RGB); the overlay/hardlight core leaves alpha = 0, so  \
           the result alpha is 0, and only the 0x00 ff000000... -> force       \
           alpha=0 */                                                          \
    ...
    void Ps##Name##Blend_HDA_o_HWY(tjs_uint32 *dest, const tjs_uint32 *src,    \
        ...
        static HWY_INLINE void PsOverlay_scalar(tjs_uint32 *dest, tjs_uint32 d,
            ...
            *dest = ps_overlay_table::TABLE[(s >> 16) & 0xff][(d >> 16) & 0xff]
                    << 16 |
        ...
        static HWY_INLINE void PsHardLight_scalar(tjs_uint32 *dest,
                                                  tjs_uint32 d, tjs_uint32 s,
                                                  tjs_uint32 a) {
            tjs_uint32 bl =
                ps_overlay_table::TABLE[(d >> 16) & 0xff][(s >> 16) & 0xff]
                    << 16 |
        ...
        static HWY_INLINE void PsExclusion_scalar(tjs_uint32 *dest,
                                                  tjs_uint32 d, tjs_uint32 s,
                                                  tjs_uint32 a) {
            tjs_uint32 sd1, sd2;
            sd1 =
                (((((d >> 16) & 0xff) * ((s & 0x00ff0000) >> 7)) & 0x01ff0000) |
        ...
        static HWY_INLINE void PsSoftLight_scalar(tjs_uint32 *dest,
                                                  tjs_uint32 d, tjs_uint32 s,
                                                  tjs_uint32 a) {
            s = (ps_soft_light_table::TABLE[(s >> 16) & 0xff][(d >> 16) & 0xff]
                 << 16) |
                (ps_soft_light_table::TABLE[(s >> 8) & 0xff][(d >> 8) & 0xff]
                 << 8) |
                (ps_soft_light_table::TABLE[s & 0xff][d & 0xff]);
            *dest = PsAlphaBlendScalar(d, s, a);
        }

        static HWY_INLINE void PsColorDodge_scalar(tjs_uint32 *dest,
                                                   tjs_uint32 d, tjs_uint32 s,
                                                   tjs_uint32 a) {
            s = (ps_color_dodge_table::TABLE[(s >> 16) & 0xff][(d >> 16) & 0xff]
                 << 16) |
        ...
        static HWY_INLINE void PsColorBurn_scalar(tjs_uint32 *dest,
                                                  tjs_uint32 d, tjs_uint32 s,
                                                  tjs_uint32 a) {
            s = (ps_color_burn_table::TABLE[(s >> 16) & 0xff][(d >> 16) & 0xff]
                 << 16) |
                (ps_color_burn_table::TABLE[(s >> 8) & 0xff][(d >> 8) & 0xff]
                 << 8) |
                (ps_color_burn_table::TABLE[s & 0xff][d & 0xff]);
            *dest = PsAlphaBlendScalar(d, s, a);
        }

        static HWY_INLINE void PsColorDodge5_scalar(tjs_uint32 *dest,
                                                    tjs_uint32 d, tjs_uint32 s,
                                                    tjs_uint32 a) {
            s = ((((s & 0x00ff00ff) * a) >> 8) & 0x00ff00ff) |
                ((((s & 0x0000ff00) * a) >> 8) & 0x0000ff00);
            *dest =
                (ps_color_dodge_table::TABLE[(s >> 16) & 0xff][(d >> 16) & 0xff]
                 << 16) |
                (ps_color_dodge_table::TABLE[(s >> 8) & 0xff][(d >> 8) & 0xff]
                 << 8) |
                (ps_color_dodge_table::TABLE[s & 0xff][d & 0xff]);
...
// These use per-pixel scalar table lookups - still vectorize the alpha blend
// step
// =========================================================================
