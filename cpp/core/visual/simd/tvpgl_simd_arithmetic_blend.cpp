...
namespace krkr2 {
    namespace HWY_NAMESPACE {
        ...
        void AddBlend_HWY(tjs_uint32 *dest, const tjs_uint32 *src,
                          tjs_int len) {
        ...
        void AddBlend_HDA_HWY(tjs_uint32 *dest, const tjs_uint32 *src,
                              tjs_int len) {
            const hn::ScalableTag<uint8_t> d8;
            const size_t N8 = hn::Lanes(d8);
            const size_t N_PIXELS = N8 / 4;
            ...
            for(; i + static_cast<tjs_int>(N_PIXELS) <= len; i += N_PIXELS) {
                ...
                auto result = hn::SaturatedAdd(vd, vs);
                // Preserve dest alpha
                result = hn::Or(hn::AndNot(alpha_mask, result),
                                hn::And(alpha_mask, vd));
                hn::StoreU(result, d8, reinterpret_cast<uint8_t *>(dest + i));
            }
        ...
        void SubBlend_HWY(tjs_uint32 *dest, const tjs_uint32 *src,
                          tjs_int len) {
            const hn::ScalableTag<uint8_t> d8;
            const hn::Repartition<uint16_t, decltype(d8)> d16;
            ...
            for(; i + static_cast<tjs_int>(N_PIXELS) <= len; i += N_PIXELS) {
                ...
                const auto half = hn::Half<decltype(d8)>();
                auto sum_lo =
                    hn::Add(hn::PromoteTo(d16, hn::LowerHalf(half, vs)),
                            hn::PromoteTo(d16, hn::LowerHalf(half, vd)));
                auto sum_hi = hn::Add(hn::PromoteUpperTo(d16, vs),
                                      hn::PromoteUpperTo(d16, vd));
                auto r_lo = hn::IfThenElse(
                    hn::Gt(sum_lo, v255), hn::Sub(sum_lo, v255), hn::Zero(d16));
                auto r_hi = hn::IfThenElse(
                    hn::Gt(sum_hi, v255), hn::Sub(sum_hi, v255), hn::Zero(d16));
            ...
            for(; i < len; i++) {
                tjs_uint32 s = src[i], d = dest[i];
                tjs_uint32 tmp =
                    ((s & d) + (((s ^ d) >> 1) & 0x7f7f7f7f)) & 0x80808080;
                tmp = (tmp << 1) - (tmp >> 7);
                dest[i] = (s + d - tmp) & tmp;
            }
        }

        void SubBlend_HDA_HWY(tjs_uint32 *dest, const tjs_uint32 *src,
                              tjs_int len) {
            const hn::ScalableTag<uint8_t> d8;
            const hn::Repartition<uint16_t, decltype(d8)> d16;
            const size_t N_PIXELS = hn::Lanes(d8) / 4;
        ...
        void SubBlend_o_HWY(tjs_uint32 *dest, const tjs_uint32 *src,
            ...
            const auto vopa16 = hn::Set(d16, static_cast<uint16_t>(opa));
            const auto v255 = hn::Set(d16, static_cast<uint16_t>(255));
            // Scalar TVPSubBlend_o_c scales only bytes 0..2 by opa and resets
            // the alpha byte to 0xFF: the saturating subtract then yields dest
            // alpha unchanged.
            const auto alpha_mask = hn::Dup128VecFromValues(
                d8, 0, 0, 0, 0xFF, 0, 0, 0, 0xFF, 0, 0, 0, 0xFF, 0, 0, 0, 0xFF);
            ...
            for(; i + static_cast<tjs_int>(N_PIXELS) <= len; i += N_PIXELS) {
                ...
                // scalar: s = ~((~src * opa) >> 8); result = max(s + d - 255,
                // 0)
                auto vs_inv = hn::Not(vs);
                const auto half = hn::Half<decltype(d8)>();
                auto si_lo = hn::ShiftRight<8>(hn::Mul(
                    hn::PromoteTo(d16, hn::LowerHalf(half, vs_inv)), vopa16));
                auto si_hi = hn::ShiftRight<8>(
                    hn::Mul(hn::PromoteUpperTo(d16, vs_inv), vopa16));
                auto vs_scaled =
                    hn::Not(hn::OrderedDemote2To(d8, si_lo, si_hi));

                auto sum_lo =
                    hn::Add(hn::PromoteTo(d16, hn::LowerHalf(half, vs_scaled)),
                            hn::PromoteTo(d16, hn::LowerHalf(half, vd)));
        ...
        void MulBlend_HWY(tjs_uint32 *dest, const tjs_uint32 *src,
                          tjs_int len) {
            const hn::ScalableTag<uint8_t> d8;
            const hn::Repartition<uint16_t, decltype(d8)> d16;
            const size_t N_PIXELS = hn::Lanes(d8) / 4;
        ...
        void MulBlend_HDA_HWY(tjs_uint32 *dest, const tjs_uint32 *src,
                              tjs_int len) {
            const hn::ScalableTag<uint8_t> d8;
            const hn::Repartition<uint16_t, decltype(d8)> d16;
            const size_t N_PIXELS = hn::Lanes(d8) / 4;
        ...
        void ScreenBlend_HWY(tjs_uint32 *dest, const tjs_uint32 *src,
                             tjs_int len) {
            const hn::ScalableTag<uint8_t> d8;
            const hn::Repartition<uint16_t, decltype(d8)> d16;
            const size_t N_PIXELS = hn::Lanes(d8) / 4;
            // Scalar TVPScreenBlend_c forces alpha byte = 0xFF (its packed
            // product only covers bytes 0..2; the alpha byte is never written,
            // so ~(0) = 0xFF).
            ...
            for(; i + static_cast<tjs_int>(N_PIXELS) <= len; i += N_PIXELS) {
                ...
                auto result = hn::Or(
                    hn::Not(hn::OrderedDemote2To(d8, r_lo, r_hi)), alpha_ff);
                hn::StoreU(result, d8, reinterpret_cast<uint8_t *>(dest + i));
            }
        ...
        void ScreenBlend_HDA_HWY(tjs_uint32 *dest, const tjs_uint32 *src,
                                 tjs_int len) {
            const hn::ScalableTag<uint8_t> d8;
            const hn::Repartition<uint16_t, decltype(d8)> d16;
            const size_t N_PIXELS = hn::Lanes(d8) / 4;
        ...
        void ScreenBlend_o_HWY(tjs_uint32 *dest, const tjs_uint32 *src,
            ...
            for(; i + static_cast<tjs_int>(N_PIXELS) <= len; i += N_PIXELS) {
                ...
                // scalar TVPScreenBlend_o_c: result = ((~d) * ~((s*opa)>>8)) >>
                // 8, alpha = 0
                const auto half = hn::Half<decltype(d8)>();
                auto s_lo = hn::ShiftRight<8>(hn::Mul(
                    hn::PromoteTo(d16, hn::LowerHalf(half, vs)), vopa16));
                auto s_hi = hn::ShiftRight<8>(
                    hn::Mul(hn::PromoteUpperTo(d16, vs), vopa16));
                auto s_scaled =
                    hn::OrderedDemote2To(d8, s_lo, s_hi); // (s*opa)>>8
                ...
                // Note: unlike HDA_o, the non-HDA scalar returns tmp (NOT
                // ~tmp), and writes alpha = 0.
                auto result = hn::OrderedDemote2To(d8, r_lo, r_hi);
