    ...
    namespace HWY_NAMESPACE {
        ...
        void SwapLine32_HWY(tjs_uint32 *line1, tjs_uint32 *line2, tjs_int len) {
            ...
            for(; i + static_cast<tjs_int>(N_PIXELS) <= len; i += N_PIXELS) {
                auto v1 =
                    hn::LoadU(d8, reinterpret_cast<const uint8_t *>(line1 + i));
                auto v2 =
                    hn::LoadU(d8, reinterpret_cast<const uint8_t *>(line2 + i));
        ...
        // =========================================================================
        // TVPMakeAlphaFromKey: set alpha=0 for pixels matching key, alpha=0xFF
        // otherwise
        // =========================================================================
        void MakeAlphaFromKey_HWY(tjs_uint32 *dest, tjs_int len,
                                  tjs_uint32 key) {
            ...
            for(; i + static_cast<tjs_int>(N) <= len; i += N) {
                auto vp = hn::LoadU(
                    d32, reinterpret_cast<const uint32_t *>(dest + i));
        ...
        // =========================================================================
        void CopyMask_HWY(tjs_uint32 *dest, const tjs_uint32 *src,
                          tjs_int len) {
            const hn::ScalableTag<uint8_t> d8;
            const size_t N_PIXELS = hn::Lanes(d8) / 4;
            const auto mask = hn::Dup128VecFromValues(
        ...
        // =========================================================================
        void CopyColor_HWY(tjs_uint32 *dest, const tjs_uint32 *src,
                           tjs_int len) {
            const hn::ScalableTag<uint8_t> d8;
            const size_t N_PIXELS = hn::Lanes(d8) / 4;
            const auto mask = hn::Dup128VecFromValues(
        ...
        // =========================================================================
        void BindMaskToMain_HWY(tjs_uint32 *main, const tjs_uint8 *mask,
                                tjs_int len) {
            for(tjs_int i = 0; i < len; i++) {
                main[i] = (main[i] & 0x00FFFFFF) |
                    (static_cast<tjs_uint32>(mask[i]) << 24);
            }
        }
        ...
        void ConstColorAlphaBlend_HWY(tjs_uint32 *dest, tjs_int len,
            ...
            const auto vcolor = hn::Dup128VecFromValues(
                d8, color_bytes[0], color_bytes[1], color_bytes[2],
                color_bytes[3], color_bytes[0], color_bytes[1], color_bytes[2],
                color_bytes[3], color_bytes[0], color_bytes[1], color_bytes[2],
                color_bytes[3], color_bytes[0], color_bytes[1], color_bytes[2],
                color_bytes[3]);
            ...
            for(; i < len; i++) {
                ...
                dest[i] = ((((((color & 0x00ff00ff) - d1) * opa) >> 8) + d1) &
                           0x00ff00ff) |
                    ((((((color & 0x0000ff00) - d2) * opa) >> 8) + d2) &
                     0x0000ff00);
            }
        }
        ...
        // =========================================================================
        void RemoveConstOpacity_HWY(tjs_uint32 *dest, tjs_int len,
                                    tjs_int strength) {
...
void TVPConstColorAlphaBlend_hwy(tjs_uint32 *dest, tjs_int len,
                                 tjs_uint32 color, tjs_int opa) {
    krkr2::HWY_DYNAMIC_DISPATCH(ConstColorAlphaBlend_HWY)(dest, len, color,
                                                          opa);
}
void TVPRemoveConstOpacity_hwy(tjs_uint32 *dest, tjs_int len,
                               tjs_int strength) {
