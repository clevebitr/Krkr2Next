...
namespace krkr2 {
    namespace HWY_NAMESPACE {
        ...
        // Split >>8 before add to avoid u16 overflow (s*a + d*inv_a can exceed
        // 65535)
        ...
        void ConstAlphaBlend_HWY(tjs_uint32 *dest, const tjs_uint32 *src,
            ...
            for(; i < len; i++) {
                ...
                dest[i] =
                    d1 | ((d2 + (((s & 0xff00) - d2) * opa >> 8)) & 0xff00);
            }
        }
        ...
        void AlphaColorMat_HWY(tjs_uint32 *dest, const tjs_uint32 color,
                               tjs_int len) {
            const hn::ScalableTag<uint8_t> d8;
            const hn::Repartition<uint16_t, decltype(d8)> d16;
            const size_t N_PIXELS = hn::Lanes(d8) / 4;
            ...
            tjs_uint32 color_arr[4] = { color_px, color_px, color_px,
                                        color_px };
            const auto vc =
                hn::LoadU(d8, reinterpret_cast<const uint8_t *>(color_arr));
