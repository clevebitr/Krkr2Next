    ...
    namespace HWY_NAMESPACE {
        ...
        void ConvertAdditiveAlphaToAlpha_HWY(tjs_uint32 *buf, tjs_int len) {
            for(tjs_int i = 0; i < len; i++) {
                ...
                buf[i] = (px & 0xff000000) + (t[(px >> 16) & 0xff] << 16) +
                    (t[(px >> 8) & 0xff] << 8) + (t[px & 0xff]);
            }
        }
        ...
        void ConvertAlphaToAdditiveAlpha_HWY(tjs_uint32 *buf, tjs_int len) {
            ...
            for(; i + static_cast<tjs_int>(N_PIXELS) <= len; i += N_PIXELS) {
                auto vp =
                    hn::LoadU(d8, reinterpret_cast<const uint8_t *>(buf + i));
                ...
                result = hn::Or(hn::AndNot(alpha_mask, result),
                                hn::And(alpha_mask, vp));
                hn::StoreU(result, d8, reinterpret_cast<uint8_t *>(buf + i));
            }
            for(; i < len; i++) {
                tjs_uint32 px = buf[i];
                tjs_uint32 a = px >> 24;
                buf[i] = (a << 24) | ((((px >> 16) & 0xff) * a / 255) << 16) |
                    ((((px >> 8) & 0xff) * a / 255) << 8) |
                    (((px) & 0xff) * a / 255);
            }
        }
        ...
        // =========================================================================
        void ReverseRGB_HWY(tjs_uint32 *dest, const tjs_uint32 *src,
                            tjs_int len) {
            const hn::ScalableTag<uint8_t> d8;
            const size_t N_PIXELS = hn::Lanes(d8) / 4;
            // Shuffle: swap byte 0 ↔ byte 2 within each 4-byte pixel
            const auto shuffle = hn::Dup128VecFromValues(
                d8, 2, 1, 0, 3, 6, 5, 4, 7, 10, 9, 8, 11, 14, 13, 12, 15);
            ...
            for(; i < len; i++) {
                tjs_uint32 p = src[i];
                dest[i] = (p & 0xFF00FF00) | ((p & 0x00FF0000) >> 16) |
                    ((p & 0x000000FF) << 16);
            }
        }
