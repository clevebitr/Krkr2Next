 ...
 *
 * Implements candidate SIMD PS blend modes; dispatch requires scalar
 * validation. 实现候选的 SIMD PS 混合模式；是否派发必须以标量验证结果为准。
 ...
 * tvpgl_simd_init.cpp 的注释：ps_alpha_blend 标量是 32
 * 位打包算术（跨字节借位）， 本 SIMD 用逐字节 u16 +
 * OrderedDemote2To，结构性无法与标量位级一致，导致 tvpgl_simd_compare
 * 失败。待用 u32 lane 逐像素重写后再放回。
 */
...
namespace krkr2 {
    namespace HWY_NAMESPACE {
        ...
        // Applies: result = d + (s - d) * a >> 8  (per channel, preserving
        // alpha) All inputs are u8 vectors. alpha must be broadcast per-pixel.
        // =========================================================================
        static HWY_INLINE hn::Vec<hn::ScalableTag<uint8_t>>
        PsApplyAlpha(hn::ScalableTag<uint8_t> d8,
            ...
            // This packed arithmetic is only an implementation candidate;
            // compare it with the scalar path before enabling dispatch.
            // 这里只是 packed
            // 算术候选实现；启用派发前必须与标量路径逐像素对比。
            auto diff_lo = hn::Sub(s_lo, d_lo);
            auto diff_hi = hn::Sub(s_hi, d_hi);
            auto r_lo =
                hn::Add(hn::ShiftRight<8>(hn::Mul(diff_lo, a_lo)), d_lo);
            auto r_hi =
                hn::Add(hn::ShiftRight<8>(hn::Mul(diff_hi, a_hi)), d_hi);
        ...
        static HWY_INLINE hn::Vec<hn::ScalableTag<uint8_t>>
        ExtractAlpha(hn::ScalableTag<uint8_t> d8,
                     hn::Vec<hn::ScalableTag<uint8_t>> vs) {
            const auto alpha_shuffle = hn::Dup128VecFromValues(
                d8, 3, 3, 3, 3, 7, 7, 7, 7, 11, 11, 11, 11, 15, 15, 15, 15);
            return hn::TableLookupBytes(vs, alpha_shuffle);
        }

        // Helper: Scale alpha by opacity
        static HWY_INLINE hn::Vec<hn::ScalableTag<uint8_t>>
        ScaleAlpha(hn::ScalableTag<uint8_t> d8,
                   hn::Vec<hn::ScalableTag<uint8_t>> va,
                   hn::Vec<hn::ScalableTag<uint16_t>> vopa16) {
            const hn::Repartition<uint16_t, decltype(d8)> d16;
            const auto half = hn::Half<decltype(d8)>();
            auto a_lo = hn::ShiftRight<8>(
                hn::Mul(hn::PromoteTo(d16, hn::LowerHalf(half, va)), vopa16));
            auto a_hi =
                hn::ShiftRight<8>(hn::Mul(hn::PromoteUpperTo(d16, va), vopa16));
            return hn::OrderedDemote2To(d8, a_lo, a_hi);
        }

        // Helper: Per-channel multiply (d * s) >> 8 in u16
        static HWY_INLINE hn::Vec<hn::ScalableTag<uint8_t>>
        MulChannels(hn::ScalableTag<uint8_t> d8,
                    hn::Vec<hn::ScalableTag<uint8_t>> va,
        ...
        static HWY_INLINE hn::Vec<hn::ScalableTag<uint8_t>>
        GetAlphaMask(hn::ScalableTag<uint8_t> d8) {
            return hn::Dup128VecFromValues(d8, 0, 0, 0, 0xFF, 0, 0, 0, 0xFF, 0,
                                           0, 0, 0xFF, 0, 0, 0, 0xFF);
        }

        // Helper: Apply HDA
        static HWY_INLINE hn::Vec<hn::ScalableTag<uint8_t>>
        ApplyHDA(hn::ScalableTag<uint8_t> d8,
        ...
        // BLEND_CORE(d8, vs, vd) must produce the blended color before alpha
        // application
        // =========================================================================
...
#define DEFINE_PS_BLEND_4VARIANTS(Name, blend_core_body)                       \
    ...
    void Ps##Name##Blend_HWY(tjs_uint32 *dest, const tjs_uint32 *src,          \
        ...
        const auto rgb_mask = hn::Dup128VecFromValues(                         \
            d8, 0xFF, 0xFF, 0xFF, 0x00, 0xFF, 0xFF, 0xFF, 0x00, 0xFF, 0xFF,    \
            0xFF, 0x00, 0xFF, 0xFF, 0xFF, 0x00);                               \
        tjs_int i = 0;                                                         \
        for(; i + static_cast<tjs_int>(N_PIXELS) <= len; i += N_PIXELS) {      \
            auto vs =                                                          \
                hn::LoadU(d8, reinterpret_cast<const uint8_t *>(src + i));     \
            auto vd =                                                          \
                hn::LoadU(d8, reinterpret_cast<const uint8_t *>(dest + i));    \
    ...
    void Ps##Name##Blend_HDA_HWY(tjs_uint32 *dest, const tjs_uint32 *src,      \
        ...
        for(; i + static_cast<tjs_int>(N_PIXELS) <= len; i += N_PIXELS) {      \
            ...
            auto vs_blended = [&]() { blend_core_body }();                     \
            auto result =                                                      \
                ApplyHDA(d8, PsApplyAlpha(d8, vd, vs_blended, va), vd);        \
            hn::StoreU(result, d8, reinterpret_cast<uint8_t *>(dest + i));     \
        }                                                                      \
        for(; i < len; i++) {                                                  \
            tjs_uint32 s = src[i], d = dest[i];                                \
    ...
    void Ps##Name##Blend_HDA_o_HWY(tjs_uint32 *dest, const tjs_uint32 *src,    \
        ...
        static HWY_INLINE void TVPPsAlphaBlend_Alpha_scalar(tjs_uint32 *dest,
                                                            tjs_uint32 d,
                                                            tjs_uint32 s,
                                                            tjs_uint32 a) {
        ...
        static HWY_INLINE void TVPPsAlphaBlend_Add_scalar(tjs_uint32 *dest,
                                                          tjs_uint32 d,
                                                          tjs_uint32 s,
                                                          tjs_uint32 a) {
            tjs_uint32 n =
                (((d & s) << 1) + ((d ^ s) & 0x00fefefe)) & 0x01010100;
        ...
        // PsSub: linear burn
        static HWY_INLINE void TVPPsAlphaBlend_Sub_scalar(tjs_uint32 *dest,
                                                          tjs_uint32 d,
                                                          tjs_uint32 s,
                                                          tjs_uint32 a) {
            tjs_uint32 si = ~s;
            tjs_uint32 n =
                (((~d & si) << 1) + ((~d ^ si) & 0x00fefefe)) & 0x01010100;
            n = ((n >> 8) + 0x007f7f7f) ^ 0x007f7f7f;
            s = (d | n) - (si | n);
            TVPPsAlphaBlend_Alpha_scalar(dest, d, s, a);
        }
        ...
        // PsMul: multiply
        static HWY_INLINE void TVPPsAlphaBlend_Mul_scalar(tjs_uint32 *dest,
                                                          tjs_uint32 d,
                                                          tjs_uint32 s,
                                                          tjs_uint32 a) {
        ...
        // PsScreen: s - s*d/255
        static HWY_INLINE void TVPPsAlphaBlend_Screen_scalar(tjs_uint32 *dest,
                                                             tjs_uint32 d,
                                                             tjs_uint32 s,
                                                             tjs_uint32 a) {
            tjs_uint32 sd1 =
                (((((d >> 16) & 0xff) * (s & 0x00ff0000)) & 0xff000000) |
                 ((((d >> 0) & 0xff) * (s & 0x000000ff)))) >>
                8;
            tjs_uint32 sd2 =
                (((((d >> 8) & 0xff) * (s & 0x0000ff00)) & 0x00ff0000)) >> 8;
            *dest =
                ((((((s & 0x00ff00ff) - sd1) * a) >> 8) + (d & 0x00ff00ff)) &
                 0x00ff00ff) |
        ...
        // PsLighten: max(d, s)
        static HWY_INLINE void TVPPsAlphaBlend_Lighten_scalar(tjs_uint32 *dest,
                                                              tjs_uint32 d,
                                                              tjs_uint32 s,
                                                              tjs_uint32 a) {
            tjs_uint32 n =
        ...
        // PsDarken: min(d, s)
        static HWY_INLINE void TVPPsAlphaBlend_Darken_scalar(tjs_uint32 *dest,
                                                             tjs_uint32 d,
                                                             tjs_uint32 s,
                                                             tjs_uint32 a) {
            tjs_uint32 n =
        ...
        // PsDiff: |d - s|
        static HWY_INLINE void TVPPsAlphaBlend_Diff_scalar(tjs_uint32 *dest,
                                                           tjs_uint32 d,
                                                           tjs_uint32 s,
                                                           tjs_uint32 a) {
            tjs_uint32 n =
        ...
        // =========================================================================
        DEFINE_PS_BLEND_4VARIANTS(Alpha, return vs;)

        // =========================================================================
        // 2. PsAddBlend: blended = SaturatedAdd(d, s)
        // =========================================================================
        DEFINE_PS_BLEND_4VARIANTS(Add, return hn::SaturatedAdd(vd, vs);)

        // =========================================================================
        // 3. PsSubBlend: Linear Burn = d - ~s (clamped)
        //    Original: n = carry; s = (d | n) - (~s | n)
        //    Simplified SIMD: SaturatedSub(d, ~s) then the result goes through
        //    alpha Actually from the scalar: s_new = (d|n) - (~s|n) which is
        //    clamp(d - ~s, 0)
        // =========================================================================
        DEFINE_PS_BLEND_4VARIANTS(Sub, auto si = hn::Not(vs);
                                  return hn::SaturatedSub(vd, si);)

        // =========================================================================
        // 4. PsMulBlend: blended = (d * s) >> 8 per channel
        // =========================================================================
        DEFINE_PS_BLEND_4VARIANTS(Mul, return MulChannels(d8, vd, vs);)
        ...
        // =========================================================================
        void PsScreenBlend_HWY(tjs_uint32 *dest, const tjs_uint32 *src,
                               tjs_int len) {
            const hn::ScalableTag<uint8_t> d8;
            const hn::Repartition<uint16_t, decltype(d8)> d16;
            const size_t N_PIXELS = hn::Lanes(d8) / 4;
            ...
            for(; i + static_cast<tjs_int>(N_PIXELS) <= len; i += N_PIXELS) {
                ...
                // Final: result = d + (s - sd - d) * a >> 8 = d + ((s - sd) -
                // d) * a >> 8 But PsScreen formula is: result_ch = d + (s - sd)
                // * a >> 8 - d * a >> 8 + d Actually: result = ((s - sd - d) *
                // a >> 8) + d Which is just: PsApplyAlpha(d, s - sd, a)
                auto s_minus_sd = hn::SaturatedSub(vs, sd);

                // ps_alpha_blend: result = d + (s_eff - d) * a >> 8
                // where s_eff = s - s*d/255
                // We need to compute this differently because s_eff is not
                // simply s Actually the correct formula from scalar:
                //   result = (((s - sd) * a >> 8) + d) for each channel
                // This is equivalent to PsApplyAlpha with s_effective = s - sd
                // But s - sd can exceed 255, but since both are u8, saturated
                // sub handles it

                const auto half = hn::Half<decltype(d8)>();
                auto se_lo =
                    hn::PromoteTo(d16, hn::LowerHalf(half, s_minus_sd));
                ...
                auto r_lo =
                    hn::Add(d_lo, hn::ShiftRight<8>(hn::Mul(se_lo, a_lo)));
                auto r_hi =
                    hn::Add(d_hi, hn::ShiftRight<8>(hn::Mul(se_hi, a_hi)));
        ...
        void PsScreenBlend_o_HWY(tjs_uint32 *dest, const tjs_uint32 *src,
            ...
            for(; i + static_cast<tjs_int>(N_PIXELS) <= len; i += N_PIXELS) {
                ...
                auto result =
                    hn::And(hn::OrderedDemote2To(d8, r_lo, r_hi), rgb_mask);
                hn::StoreU(result, d8, reinterpret_cast<uint8_t *>(dest + i));
            }
        ...
        void PsScreenBlend_HDA_HWY(tjs_uint32 *dest, const tjs_uint32 *src,
            ...
            for(; i + static_cast<tjs_int>(N_PIXELS) <= len; i += N_PIXELS) {
                ...
                auto result =
                    ApplyHDA(d8, hn::OrderedDemote2To(d8, r_lo, r_hi), vd);
                hn::StoreU(result, d8, reinterpret_cast<uint8_t *>(dest + i));
            }
        ...
        // =========================================================================
        DEFINE_PS_BLEND_4VARIANTS(Lighten, return hn::Max(vd, vs);)

        // =========================================================================
        // 7. PsDarkenBlend: blended = min(d, s) per channel
        // =========================================================================
        DEFINE_PS_BLEND_4VARIANTS(Darken, return hn::Min(vd, vs);)
...
#define EXPORT_PS_BLEND(Name)                                                  \
    ...
    void TVPPs##Name##Blend_o_hwy(tjs_uint32 *dest, const tjs_uint32 *src,     \
                                  tjs_int len, tjs_int opa) {                  \
        krkr2::HWY_DYNAMIC_DISPATCH(Ps##Name##Blend_o_HWY)(dest, src, len,     \
                                                           opa);               \
    }                                                                          \
    ...
    void TVPPs##Name##Blend_HDA_o_hwy(tjs_uint32 *dest, const tjs_uint32 *src, \
                                      tjs_int len, tjs_int opa) {              \
        krkr2::HWY_DYNAMIC_DISPATCH(Ps##Name##Blend_HDA_o_HWY)(dest, src, len, \
                                                               opa);           \
    }
