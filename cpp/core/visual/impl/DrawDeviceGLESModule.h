...
namespace DrawDeviceGLES {
    ...
    using CreateKrkrGLESModuleObjectFn = tjs_error (*)(tTJSVariant *, tjs_int,
                                                       tjs_int);
    ...
    inline tjs_error TryCreateModuleViaKrkrGLES(tTJSVariant *result,
    ...
    inline tjs_error ReturnFirstArgOrTrueCb(tTJSVariant *result,
                                            tjs_int numparams,
                                            tTJSVariant **param,
                                            iTJSDispatch2 *) {
        if(!result)
    ...
    inline tjs_error CreateFallbackModuleObject(tTJSVariant *result,
                                                tjs_int width, tjs_int height) {
        ...
        dict->PropSet(TJS_MEMBERENSURE, TJS_W("screenWidth"), nullptr, &wv,
                      dict);
        dict->PropSet(TJS_MEMBERENSURE, TJS_W("screenHeight"), nullptr, &hv,
                      dict);
    ...
    inline ModuleName NormalizeModuleName(tjs_int numparams,
                                          tTJSVariant **param) {
        ...
        for(auto &ch : out) {
            if(ch >= static_cast<tjs_char>('A') &&
               ch <= static_cast<tjs_char>('Z')) {
    ...
    tjs_error GetModuleForDevice(DeviceT *device, tTJSVariant *result,
        ...
        if(moduleName == ModuleName(TJS_W("live2d")))
            EnsureWindowGLESAdaptor(device);
        ...
        if(TJS_SUCCEEDED(TryCreateModuleViaKrkrGLES(&created, w, h))) {
            if(created.Type() == tvtObject &&
               created.AsObjectNoAddRef() != nullptr)
