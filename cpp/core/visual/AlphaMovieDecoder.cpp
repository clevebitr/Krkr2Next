...
bool DecodeFrameToRgba(uint8_t qtbl[3][64], bool isZlib, uint32_t zlibBufSize,
    ...
    if(!payload || geo.width == 0 || geo.height == 0)
        return false;
    EnsureAlphaMovieTablesInitialized();

    size_t payloadOff = 0;
    static SwsContext *s_convertCtx = nullptr;
    std::vector<uint8_t> scratchCache;
    std::vector<uint8_t> scratchAlpha;
    ...
    if(isZlib) { // zlib 解压缩
        ...
        payloadOff += (cacheLen);
            ...
            // yuv
            cacheLen = payloadLen - zlibBufSize;
            scratchCache.resize(cacheLen);
        ...
        payloadOff += (cacheLen);
            ...
            tjs_uint8* uStart = yuv_buffer + geo.width * geo.height;
            tjs_uint8* vStart = uStart + geo.width * geo.height / 4;

            // jpeg decode
            auto streamOwner = std::make_unique<BufferManager>(cache, cacheLen);
            BufferManager* stream = streamOwner.get();
            scratchIdc.resize(64);
            int16_t* idcBuff = scratchIdc.data();
            int decoded = 0;
            // 8*8分块处理, U/V单独算一个小块，2*2Y算一个小块
            for (size_t i = 0; i < geo.height / 16; i++)
            {
                for (size_t j = 0; j < geo.width / 16; j++)
                {
                    // Pose
                    tjs_uint8* currentY = yuv_buffer + geo.width * 16 * i + 16 * j;
                    tjs_uint8* currentU = uStart + geo.width * 4 * i + 8 * j;
                    tjs_uint8* currentV = vStart + geo.width * 4 * i + 8 * j;
                    ...
                    if (decoded == 1)
                    {
                        ...
                        *((uint64_t*)(currentU + geo.width / 2)) = pattern;
                        *((uint64_t*)(currentU + geo.width / 2 * 2)) = pattern;
                        *((uint64_t*)(currentU + geo.width / 2 * 3)) = pattern;
                        *((uint64_t*)(currentU + geo.width / 2 * 4)) = pattern;
                        *((uint64_t*)(currentU + geo.width / 2 * 5)) = pattern;
                        *((uint64_t*)(currentU + geo.width / 2 * 6)) = pattern;
                        *((uint64_t*)(currentU + geo.width / 2 * 7)) = pattern;
                    }
                    else
                    {
                        jpeg_idct_data(idcBuff, qtbl[1], currentU, geo.width / 2);
                    }
                    ...
                    if (decoded == 1)
                    {
                        ...
                        *((uint64_t*)(currentV + geo.width / 2)) = pattern;
                        *((uint64_t*)(currentV + geo.width / 2 * 2)) = pattern;
                        *((uint64_t*)(currentV + geo.width / 2 * 3)) = pattern;
                        *((uint64_t*)(currentV + geo.width / 2 * 4)) = pattern;
                        *((uint64_t*)(currentV + geo.width / 2 * 5)) = pattern;
                        *((uint64_t*)(currentV + geo.width / 2 * 6)) = pattern;
                        *((uint64_t*)(currentV + geo.width / 2 * 7)) = pattern;
                    }
                    else
                    {
                        jpeg_idct_data(idcBuff, qtbl[1], currentV, geo.width / 2);
                    }
                    ...
                    if (decoded == 1)
                    {
                        ...
                        *((uint64_t*)(currentY + geo.width)) = pattern;
                        *((uint64_t*)(currentY + geo.width * 2)) = pattern;
                        *((uint64_t*)(currentY + geo.width * 3)) = pattern;
                        *((uint64_t*)(currentY + geo.width * 4)) = pattern;
                        *((uint64_t*)(currentY + geo.width * 5)) = pattern;
                        *((uint64_t*)(currentY + geo.width * 6)) = pattern;
                        *((uint64_t*)(currentY + geo.width * 7)) = pattern;
                    }
                    else
                    {
                        jpeg_idct_data(idcBuff, qtbl[0], currentY, geo.width);
                    }
                    memset(idcBuff, 0, 128);
                    decode_dc_run_length(&HuffmanTable[2], stream, idcBuff, stream->decodeYStatus);
                    decoded = decode_ac_run_length(&HuffmanTable[3], stream, idcBuff);
                    if (decoded == 1)
                    {
                        ...
                        *((uint64_t*)(currentY + 8 + geo.width)) = pattern;
                        *((uint64_t*)(currentY + 8 + geo.width * 2)) = pattern;
                        *((uint64_t*)(currentY + 8 + geo.width * 3)) = pattern;
                        *((uint64_t*)(currentY + 8 + geo.width * 4)) = pattern;
                        *((uint64_t*)(currentY + 8 + geo.width * 5)) = pattern;
                        *((uint64_t*)(currentY + 8 + geo.width * 6)) = pattern;
                        *((uint64_t*)(currentY + 8 + geo.width * 7)) = pattern;
                    }
                    else
                    {
                        jpeg_idct_data(idcBuff, qtbl[0], currentY + 8, geo.width);
                    }
                    memset(idcBuff, 0, 128);
                    decode_dc_run_length(&HuffmanTable[2], stream, idcBuff, stream->decodeYStatus);
                    decoded = decode_ac_run_length(&HuffmanTable[3], stream, idcBuff);
                    if (decoded == 1)
                    {
                        ...
                        // 填充8x8块 - 7次内存写入覆盖整个块
                        *((uint64_t*)(currentY + geo.width * 8)) = pattern;
                        *((uint64_t*)(currentY + geo.width * 9)) = pattern;
                        *((uint64_t*)(currentY + geo.width * 10)) = pattern;
                        *((uint64_t*)(currentY + geo.width * 11)) = pattern;
                        *((uint64_t*)(currentY + geo.width * 12)) = pattern;
                        *((uint64_t*)(currentY + geo.width * 13)) = pattern;
                        *((uint64_t*)(currentY + geo.width * 14)) = pattern;
                        *((uint64_t*)(currentY + geo.width * 15)) = pattern;
                    }
                    else
                    {
                        jpeg_idct_data(idcBuff, qtbl[0], currentY + geo.width * 8,
                                       geo.width);
                    }
                    memset(idcBuff, 0, 128);
                    decode_dc_run_length(&HuffmanTable[2], stream, idcBuff, stream->decodeYStatus);
                    ...
                    if (decoded == 1)
                    {
                        ...
                        // 填充8x8块 - 7次内存写入覆盖整个块
                        *((uint64_t*)(currentY + 8 + geo.width * 8)) = pattern;
                        *((uint64_t*)(currentY + 8 + geo.width * 9)) = pattern;
                        *((uint64_t*)(currentY + 8 + geo.width * 10)) = pattern;
                        *((uint64_t*)(currentY + 8 + geo.width * 11)) = pattern;
                        *((uint64_t*)(currentY + 8 + geo.width * 12)) = pattern;
                        *((uint64_t*)(currentY + 8 + geo.width * 13)) = pattern;
                        *((uint64_t*)(currentY + 8 + geo.width * 14)) = pattern;
                        *((uint64_t*)(currentY + 8 + geo.width * 15)) = pattern;
                    }
                    else
                    {
                        jpeg_idct_data(idcBuff, qtbl[0], currentY + 8 + geo.width * 8,
                                       geo.width);
                    }
                }
            }
            // clean
            // yuv420p to rgba
            uLongf rgbaSize = geo.width * geo.height * 4;
            auto rgbaBuffer = std::make_unique<tjs_uint8[]>(rgbaSize);
            tjs_uint8* rgba_buffer = rgbaBuffer.get();
            s_convertCtx =
                sws_getCachedContext(s_convertCtx, geo.width, geo.height,
                                     AV_PIX_FMT_YUV420P, geo.width, geo.height,
                                     AV_PIX_FMT_RGBA, SWS_FAST_BILINEAR, NULL, NULL, NULL);
            if (s_convertCtx == nullptr)
                return false;
            const uint8_t* src_slice[4] = {
                yuv_buffer, yuv_buffer + geo.width * geo.height,
                yuv_buffer + geo.width * geo.height * 5 / 4, NULL};
            int src_stride[4] = {geo.width,     // Y分量步长
                                 geo.width / 2, // U分量步长（宽度减半）
                                 geo.width / 2, // V分量步长（宽度减半）
                                 0};
            uint8_t* dst[4] = {rgba_buffer, // RGBA数据
                               NULL, NULL, NULL};
            int dst_stride[4] = {geo.width * 4, // RGBA步长
                                 0, 0, 0};
            if (sws_scale(s_convertCtx, src_slice, src_stride, 0,
                          geo.height, dst, dst_stride) <= 0)
                return false;
            // clean
        ...
        payloadOff += (cacheLen);
            ...
            tjs_uint8* uStart = yuva_buffer + geo.width * geo.height;
            tjs_uint8* vStart = uStart + geo.width * geo.height / 4;
            tjs_uint8* aStart = vStart + geo.width * geo.height / 4;
            ...
            for (size_t i = 0; i < geo.height / 16; i++)
            {
                ...
                {
                    // Pose
                    tjs_uint8* currentY = yuva_buffer + geo.width * 16 * i + 16 * j;
                    tjs_uint8* currentU = uStart + geo.width * 4 * i + 8 * j;
                    tjs_uint8* currentV = vStart + geo.width * 4 * i + 8 * j;
                    tjs_uint8* currentA = aStart + geo.width * 16 * i + 16 * j;
                    ...
                    if (decoded == 1)
                    {
                        ...
                        *((uint64_t*)(currentA + geo.width)) = pattern;
                        *((uint64_t*)(currentA + geo.width * 2)) = pattern;
                        *((uint64_t*)(currentA + geo.width * 3)) = pattern;
                        *((uint64_t*)(currentA + geo.width * 4)) = pattern;
                        *((uint64_t*)(currentA + geo.width * 5)) = pattern;
                        *((uint64_t*)(currentA + geo.width * 6)) = pattern;
                        *((uint64_t*)(currentA + geo.width * 7)) = pattern;
                    }
                    else
                    {
                        jpeg_idct_data(idcBuff, qtbl[0], currentA, geo.width);
                    }
                    memset(idcBuff, 0, 128);
                    decode_dc_run_length(&HuffmanTable[2], stream, idcBuff, stream->decodeYStatus);
                    decoded = decode_ac_run_length(&HuffmanTable[3], stream, idcBuff);
                    if (decoded == 1)
                    {
                        ...
                        *((uint64_t*)(currentA + 8 + geo.width)) = pattern;
                        *((uint64_t*)(currentA + 8 + geo.width * 2)) = pattern;
                        *((uint64_t*)(currentA + 8 + geo.width * 3)) = pattern;
                        *((uint64_t*)(currentA + 8 + geo.width * 4)) = pattern;
                        *((uint64_t*)(currentA + 8 + geo.width * 5)) = pattern;
                        *((uint64_t*)(currentA + 8 + geo.width * 6)) = pattern;
                        *((uint64_t*)(currentA + 8 + geo.width * 7)) = pattern;
                    }
                    else
                    {
                        jpeg_idct_data(idcBuff, qtbl[0], currentA + 8, geo.width);
                    }
                    memset(idcBuff, 0, 128);
                    decode_dc_run_length(&HuffmanTable[2], stream, idcBuff, stream->decodeYStatus);
                    decoded = decode_ac_run_length(&HuffmanTable[3], stream, idcBuff);
                    if (decoded == 1)
                    {
                        ...
                        // 填充8x8块 - 7次内存写入覆盖整个块
                        *((uint64_t*)(currentA + geo.width * 8)) = pattern;
                        *((uint64_t*)(currentA + geo.width * 9)) = pattern;
                        *((uint64_t*)(currentA + geo.width * 10)) = pattern;
                        *((uint64_t*)(currentA + geo.width * 11)) = pattern;
                        *((uint64_t*)(currentA + geo.width * 12)) = pattern;
                        *((uint64_t*)(currentA + geo.width * 13)) = pattern;
                        *((uint64_t*)(currentA + geo.width * 14)) = pattern;
                        *((uint64_t*)(currentA + geo.width * 15)) = pattern;
                    }
                    else
                    {
                        jpeg_idct_data(idcBuff, qtbl[0], currentA + geo.width * 8,
                                       geo.width);
                    }
                    memset(idcBuff, 0, 128);
                    decode_dc_run_length(&HuffmanTable[2], stream, idcBuff, stream->decodeYStatus);
                    ...
                    if (decoded == 1)
                    {
                        ...
                        // 填充8x8块 - 7次内存写入覆盖整个块
                        *((uint64_t*)(currentA + 8 + geo.width * 8)) = pattern;
                        *((uint64_t*)(currentA + 8 + geo.width * 9)) = pattern;
                        *((uint64_t*)(currentA + 8 + geo.width * 10)) = pattern;
                        *((uint64_t*)(currentA + 8 + geo.width * 11)) = pattern;
                        *((uint64_t*)(currentA + 8 + geo.width * 12)) = pattern;
                        *((uint64_t*)(currentA + 8 + geo.width * 13)) = pattern;
                        *((uint64_t*)(currentA + 8 + geo.width * 14)) = pattern;
                        *((uint64_t*)(currentA + 8 + geo.width * 15)) = pattern;
                    }
                    else
                    {
                        jpeg_idct_data(idcBuff, qtbl[0], currentA + 8 + geo.width * 8,
                                       geo.width);
                    }
                }
            }
            ...
            s_convertCtx =
                sws_getCachedContext(s_convertCtx, geo.width, geo.height,
                                     AV_PIX_FMT_YUVA420P, geo.width, geo.height,
                                     AV_PIX_FMT_RGBA, SWS_FAST_BILINEAR, NULL, NULL, NULL);
            if (s_convertCtx == nullptr)
                return false;
            const uint8_t* src_slice[5] = {
                yuva_buffer, yuva_buffer + geo.width * geo.height,
                yuva_buffer + geo.width * geo.height * 5 / 4,
                yuva_buffer + geo.width * geo.height * 3 / 2, NULL};
            int src_stride[5] = {geo.width,     // Y分量步长
                                 geo.width / 2, // U分量步长（宽度减半）
                                 geo.width / 2, // V分量步长（宽度减半）
                                 geo.width, 0};
            uint8_t* dst[4] = {rgba_buffer, // RGBA数据
